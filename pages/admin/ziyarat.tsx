...
export default function ZiyaratPage() {
  ...
  const [groups, setGroups] = useState<ZiyaratGroup[]>([]);
  const [members, setMembers] = useState<ZiyaratMember[]>([]);
  ...
  const [newGroup, setNewGroup] = useState({
    groupName: '',
    destinationType: 'Iran / Iraq / Syria',
    agentName: '',
    departureDate: '',
    returnDate: ''
  });
  ...
  const fetchData = async () => {
    ...
    try {
      const [grpRes, memRes, zairRes, pkgRes, agtRes] = await Promise.all([
        supabase.from('ziyarat_groups').select('*').order('created_at', { ascending: false }),
        supabase.from('ziyarat_members').select('*'),
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('ziyarat_groups').insert([{
        group_name: newGroup.groupName.trim(),
        destination_type: newGroup.destinationType,
      ...
      setSuccessMsg('Ziyarat Group Created Successfully!');
      setNewGroup({ groupName: '', destinationType: 'Iran / Iraq / Syria', agentName: '', departureDate: '', returnDate: '' });
      fetchData();
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to create Ziyarat Group.');
    }
  };
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('ziyarat_members').insert([{
      ...
      if (error) throw error;
      setSuccessMsg('Zair Added to Ziyarat Group Successfully!');
  ...
  const handleDeleteMember = async (id: number) => {
    if (!confirm('Remove this Zair from the Ziyarat group?')) return;
    try {
      await supabase.from('ziyarat_members').delete().eq('id', id);
      fetchData();
    } catch (err: any) {
      alert(err.message);
    }
  ...
  return (
    <>
      <Head>
        <title>Ziyarat Management - Karwan-e-Rahian-e-Noor</title>
      </Head>
      ...
      <div className="w-full space-y-4 pb-12">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="px-3.5 py-1 bg-[#0b2447] text-amber-400 font-bold text-xs rounded-full shadow-sm">
            Total Ziyarat Groups: {groups.length}
          </span>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200/80">
              <h3 className="text-xs sm:text-sm font-black text-slate-800 uppercase tracking-wider pb-3 border-b border-slate-100 flex items-center gap-2">
                ...
                <span>Create New Ziyarat Group</span>
              </h3>

              <form onSubmit={handleCreateGroup} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 pt-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Group Name *</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. Arbaeen Special 2026"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Destinations *</label>
                  <input
                    type="text"
                    required
                    value={newGroup.destinationType}
                    onChange={(e) => setNewGroup({ ...newGroup, destinationType: e.target.value })}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white focus:border-[#0b2447] outline-none"
                  />
                </div>
            ...
            {/* 2. Add Zair to Ziyarat Group Form */}
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200/80">
              <h3 className="text-xs sm:text-sm font-black text-slate-800 uppercase tracking-wider pb-3 border-b border-slate-100 flex items-center gap-2">
                <UserPlus className="w-4 h-4 text-emerald-600" />
                <span>Assign Registered Zair to Ziyarat Group & Package</span>
              </h3>

              <form onSubmit={handleAddMember} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 pt-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Select Ziyarat Group *</label>
                  <select
                    required
            ...
            {/* 3. Ziyarat Groups & Pilgrims Listing */}
            {groups.map((group) => {
              ...
              return (
                <div key={group.id} className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200/80 space-y-4">
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2 pb-3 border-b border-slate-100">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-0.5 bg-teal-100 text-teal-800 font-bold text-[10px] rounded-md">
                          {group.agent_name}
                        </span>
                        <h4 className="text-base font-black text-slate-800 uppercase tracking-tight">
                          {group.group_name} ({group.destination_type})
                        </h4>
                      </div>
                  ...
                  {groupMembers.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-3">No pilgrims added to this Ziyarat group yet.</p>
                  ) : (
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-sm border border-slate-200/80">
              <h3 className="text-xs sm:text-base font-black text-slate-800 uppercase tracking-wider pb-4 border-b border-slate-100 flex items-center gap-2">
                ...
                <span>Agent-Wise Ziyarat Group Billing & Ledger Overview</span>
              </h3>
                  ...
                  return (
                    <div key={agent.id} className="bg-slate-50 border border-slate-200/90 rounded-2xl p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="px-2.5 py-1 bg-teal-700 text-white text-[10px] font-bold rounded-lg">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {agentGroups.map(g => (
                          <span key={g.id} className="text-[10px] font-semibold px-2 py-0.5 bg-teal-100 text-teal-900 rounded-md">
                            {g.group_name}
                          </span>
                        ))}
                      </div>
